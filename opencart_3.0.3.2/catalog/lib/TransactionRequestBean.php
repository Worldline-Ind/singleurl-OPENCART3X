...
/**
    ...
    public function setHashAlgo($webHashAlgo)
    {
        $this->webHashAlgo = $webHashAlgo;
    }
    ...
    {   
        try {
            ...
            if($this->isBlankOrNull($this->webHashAlgo)){
                $webHashAlgo='SHA3-512';
               }else{
                $webHashAlgo=$this->webHashAlgo;
               }
           
            $salt = bin2hex(random_bytes(16));
            $withsalt=$this->strReqst."|".$salt;
            $sha = hash($webHashAlgo, $withsalt);
            $this->strReqst .= "|hash=" . $sha. "|hashAlgo=".$webHashAlgo."|random_salt=".$salt;
