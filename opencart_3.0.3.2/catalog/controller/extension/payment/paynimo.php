...
class ControllerExtensionPaymentPaynimo extends Controller {
	public function index() {
		...
		$cur_date = date("d-m-Y");
		$returnUrl = $this->url->link('extension/payment/paynimo/getResponse');
		...
		$transactionRequestBean->setHashAlgo($merchant_details[0]['hashalgo']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/paynimo')) {
			return $this->load->view($this->config->get('config_template') . 'extension/payment/paynimo', $data);
		} else {
			return $this->load->view('extension/payment/paynimo', $data);
		}
	}
	...
	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'paynimo') {
			$this->load->model('checkout/order');
			
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_paynimo_order_status'));
		}
	}
}